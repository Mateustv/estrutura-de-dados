...
def sring_to_integer(s):
    ...
    if jump < len(s) and s[jump] == '+':
        sinal = 1
        jump += 1
    elif jump < len(s) and s[jump] == '-':
        sinal = -1
        jump += 1

    for number in range(jump, len(s)):
        if s[number].isdigit():
            teste += s[number]
            print('tt')
            if (int(teste) <= -2 ** 31) or (int(teste) >= 2 ** 31 - 1):
                return pow(2,31) - 1  if sinal == 1 else  -pow(2,31)
            else:
                numberStr += s[number]
        else:
            break
    ans = 0
    ans = int(numberStr)*sinal
    return ans

sring_to_integer('1000000')